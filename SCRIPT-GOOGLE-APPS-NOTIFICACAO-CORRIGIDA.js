// Google Apps Script Notificação Final - NOTIFICAÇÃO CORRIGIDA
...
function enviarFechamentoCompleto(data) {
  try {
    ...
    // 7. Enviar notificação com PDF para gerentes - CORRIGIDA
...
function buscarPerdasServico(dataOperacional) {
  try {
    ...
    if (response.getResponseCode() === 200) {
      ...
      console.log('Perdas encontradas:', perdas.length);
      return perdas;
    } else {
      console.error('Erro ao buscar perdas:', response.getResponseCode());
      return [];
    }
  } catch (error) {
...
// Gerar corpo do email SEM PDF (só XLSX) - COM PERDAS DE SERVIÇO
function gerarCorpoEmailSemPDF(emailData) {
  // Buscar perdas de serviço do dia operacional
  ...
  // Adicionar seção de perdas de serviço se houver perdas
  if (perdas && perdas.length > 0) {
    corpo += '<div style="background-color: #fdf2e9; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #e67e22;">' +
      '<h3 style="color: #d35400; margin-top: 0;">⚠️ Perdas de Serviço (' + perdas.length + ')</h3>';
    ...
    // Listar perdas por atendente
    for (var atendente in perdasPorAtendente) {
      var perdasAtendente = perdasPorAtendente[atendente];
      corpo += '<div style="margin: 10px 0; padding: 10px; background-color: #fff; border-radius: 3px;">' +
        '<strong style="color: #c0392b;">' + atendente + ' (' + perdasAtendente.length + ' perda' + (perdasAtendente.length > 1 ? 's' : '') + '):</strong><br>';
      
      for (var j = 0; j < perdasAtendente.length; j++) {
        var perda = perdasAtendente[j];
        var dataPerda = new Date(perda.created_at);
        var horaPerda = dataPerda.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
        
        corpo += '<div style="margin: 5px 0 5px 15px; font-size: 14px; color: #555;">' +
          '• <strong>Mesa ' + perda.numero_mesa + '</strong> (' + horaPerda + '): ' + perda.motivo +
          '</div>';
      }
      corpo += '</div>';
    }
    
    corpo += '<p style="color: #d35400; font-size: 12px; margin: 10px 0 0 0; font-style: italic;">' +
      'Total de perdas registradas: ' + perdas.length + ' | Atendentes afetados: ' + Object.keys(perdasPorAtendente).length +
      '</p>' +
      '</div>';
  } else {
    // Se não houver perdas, mostrar mensagem positiva
    corpo += '<div style="background-color: #d5f4e6; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #27ae60;">' +
      '<h3 style="color: #27ae60; margin-top: 0;">✅ Perdas de Serviço</h3>' +
      '<p style="color: #2d5a3d; margin: 0;">Nenhuma perda de serviço registrada no dia operacional.</p>' +
      '</div>';
  }
