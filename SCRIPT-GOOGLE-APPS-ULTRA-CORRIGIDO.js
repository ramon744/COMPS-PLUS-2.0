// Google Apps Script Notificação Final - ULTRA CORRIGIDO
...
// Enviar fechamento completo - ULTRA CORRIGIDO
...
// Função para formatar data no padrão brasileiro (dd/mm/aaaa) - ULTRA CORRIGIDA
...
// Atualizar cabeçalho interno - ULTRA CORRIGIDA
function updateHeaderInterno(headerData) {
  try {
    ...
    var comps2Pct = (headerData.porcentagens.comps2 || '0%').replace('.00%', '%');
    var comps4Pct = (headerData.porcentagens.comps4 || '0%').replace('.00%', '%');
    var comps8Pct = (headerData.porcentagens.comps8 || '0%').replace('.00%', '%');
    var comps11Pct = (headerData.porcentagens.comps11 || '0%').replace('.00%', '%');
    var comps12Pct = (headerData.porcentagens.comps12 || '0%').replace('.00%', '%');
    var comps13Pct = (headerData.porcentagens.comps13 || '0%').replace('.00%', '%');
...
// Adicionar garçons interno - ULTRA CORRIGIDA
function addWaitersInterno(waitersData) {
  try {
    ...
    for (var i = 0; i < waitersData.length; i++) {
      ...
      // CORREÇÃO: Usar valores diretos dos comps, não calcular
      var comps2Valor = waiter.comps2 || '';
      var comps4Valor = waiter.comps4 || '';
      var comps8Valor = waiter.comps8 || '';
      var comps11Valor = waiter.comps11 || '';
      var comps12Valor = waiter.comps12 || '';
      var comps13Valor = waiter.comps13 || '';
